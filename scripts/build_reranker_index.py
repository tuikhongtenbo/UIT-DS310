"""
Build Reranker Index Script
Build ChromaDB index for reranker module
"""
...
logger = setup_logger("build_reranker_index")


def build_reranker_index(data, config):
    """
    Build ChromaDB index for reranker module.
    ...
    """
    logger.info("=" * 60)
    logger.info("Building Reranker ChromaDB Index")
    logger.info("=" * 60)
    
    # 1. Get reranker configuration
    reranker_config = config.get('reranker', {})
    embedder_config = reranker_config.get('embedder', {})
    
    chunk_size = embedder_config.get('chunk_size', 2048)
    overlap = embedder_config.get('overlap', 128)
    ...
    logger.info("=" * 60)
    logger.info("Reranker ChromaDB index built successfully!")
...
def main():
    """
    Main function to build reranker index.
    """
    parser = argparse.ArgumentParser(description="Build Reranker ChromaDB Index")
    parser.add_argument(
        "--reset", 
        action="store_true", 
    ...
    if args.reset:
        ...
        reranker_config = cfg.config.get('reranker', {})
        embedder_config = reranker_config.get('embedder', {})
        collection_name = embedder_config.get('collection_name', "reranker_legal_articles")
    ...
    build_reranker_index(data, cfg.config)
